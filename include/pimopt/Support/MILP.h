...
//
// This file is from project Dynamatic
//
//===----------------------------------------------------------------------===//
//
...
#ifndef PIMOPT_SUPPORT_MILP_H
#define PIMOPT_SUPPORT_MILP_H
...
public:
    ...
    MILP(GRBEnv &env, const llvm::StringRef &writeTo = "")
    ...
    mlir::LogicalResult optimize(int *milpStatus = nullptr) {
        ...
        // Check whether we reached an optimal solution or reached the time limit
        int stat = model.get(GRB_IntAttr_Status);
        if (milpStatus) *milpStatus = stat;
        if (stat != GRB_OPTIMAL && stat != GRB_TIME_LIMIT) {
            state = State::FAILED_TO_OPTIMIZE;
            llvm::errs() << "PIM Detailed Data Layout Optimization failed with status " << stat
    ...
    mlir::LogicalResult getResult() {
        if (state != State::OPTIMIZED) {
            llvm::errs() << "Optimized results can not be extracted from MILP (reason: "
                         << getStateMessage() << ").";
    ...
    void markReadyToOptimize() {
        assert(state == State::FAILED_TO_SETUP ||
               state == State::READY && "can only mark MILP ready from constructor");
    ...
    // This class manages a Gurobi model which shall not be copied, hence the copy-assignment constructor is deleted.
...
private:
    ...
    llvm::StringRef getStateMessage() {
        switch (state) {
...
mlir::LogicalResult solveMILP(MILPRes &milpResult , Args &&...args) {
    MILP milp = MILP(std::forward<Args>(args)...);
    if (mlir::failed(milp.optimize()) || mlir::failed(milp.getResult(milpResult)))
        return mlir::failure();
