...
loop_bound_name_conv2D = {"N": 0, "K": 1, "P": 2, "Q": 3, "C": 4, "R": 5, "S": 6}

loop_bound_name_FC = {"N": 0, "K": 1, "P": 2, "Q": 3, "R": 4}
...
loop_bound_idx_name_map_FC = {0: "N", 1: "K", 2: "P", 3: "Q", 4: "R"}

device_type_idx = {"PUM": 0, "NBP": 1}

device_type_idx_name_map = {0: "PUM", 1: "NBP"}
...
class ArchInfo:
    ...
    def _initialize_attributes(self, data):
        ...
        self.PEPerChannel = data.get("PEPerChannel", 16)
        
        # Number of channels
        self.numChannel = data.get("numChannel", 16)
        
        # Burst length
        self.numBL = data.get("numBL", 4)
        
        # Number of bits in a DQ
        self.DQWidth = data.get("DQWidth", 128)

        # Number of cycles needed from ACT to WRITE
        self.rowActLat = data.get("rowActLat", 24)
    ...
    def __repr__(self):
        return (
            f"ArchInfo(dataWidth={self.dataWidth} bits/element, numRow={self.numRow}, numCol={self.numCol}, "
...
def cal_performance_FC(arch_info, loop_bounds, device_type):
    ...
    num_channel_Sys = (num_PE_Sys) / arch_info.PEPerChannel
    # print("haha", num_channel_Sys)
    ChannelBandWidth = min(arch_info.SysBandWidth * num_channel_Sys, arch_info.SysBandWidth)
    ...
    op_performance.outTransCost = (float)(
        op_performance.numOutSys * arch_info.dataWidth
    ...
    op_performance.inLoadingCost = (float)(
        op_performance.numInSys * arch_info.dataWidth
    ) / (float)(ChannelBandWidth)
    
    if device_type == 1:
        num_input_reuse = loop_bounds[loop_bound_name_FC["K"]][0] * loop_bounds[loop_bound_name_FC["R"]][0]
        effective_num_PE = min(num_PE_Sys, arch_info.PEPerChannel)
        op_performance.inLoadingCost = (num_col_PE * arch_info.dataWidth * num_mul_col * effective_num_PE / (arch_info.DQWidth * num_input_reuse)) * arch_info.numBL


    # If the target is HBM-PIM
    if device_type == 1:
        filPELoading = (num_col_PE * arch_info.dataWidth * num_mul_col / arch_info.DQWidth)
    ...
    if device_type == 0:
        ...
        # final_performance += op_performance.outTransCost
        final_performance += num_mul_col * arch_info.rowActLat
        final_performance += op_performance.numFilPELoading
        op_performance.finalPerformance = final_performance
...
def cal_performance_conv2D(
    arch_info, loop_bounds, trans_coeffs, stride, dilation, device_type
):
    """
    This function calculates the final performance of the given transformed loop
    Needed Inputs:
    ...
    in_term_1 = count_unique(
        coeff_1,
        coeff_2,
        loop_bounds[loop_bound_name_conv2D["P"]][0],
        loop_bounds[loop_bound_name_conv2D["R"]][0],
    )
    ...
    in_term_2 = count_unique(
        coeff_3,
        coeff_4,
        loop_bounds[loop_bound_name_conv2D["Q"]][0],
        loop_bounds[loop_bound_name_conv2D["S"]][0],
    )
    ...
    #! New calculation for the performance cost
    num_channel_Sys = (num_PE_Sys) / arch_info.PEPerChannel
    # print("Num Channel Sys:", num_channel_Sys)
    ChannelBandWidth = min(arch_info.SysBandWidth * num_channel_Sys, arch_info.SysBandWidth)
    # print("Channel Bandwidth:", ChannelBandWidth)
    ...
    if device_type == 1:
        # Calculate input reuse
        num_input_reuse = loop_bounds[loop_bound_name_conv2D["K"]][0]
        effective_num_PE = min(num_PE_Sys, arch_info.PEPerChannel)
        op_performance.inLoadingCost = (num_col_PE * arch_info.dataWidth * num_mul_col * effective_num_PE / (arch_info.DQWidth * num_input_reuse)) * arch_info.numBL
