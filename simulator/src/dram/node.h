...
DECLARE_DEBUG_FLAG(DNODE);
ENABLE_DEBUG_FLAG(DNODE);
...
struct DRAMNodeBase {
    ...
    bool check_ready(int command, const AddrVec_t& addr_vec, Clk_t clk) {
      DEBUG_LOG(DNODE, Logging::get("Base"), "checking {} at level-{} index-{}", T::m_commands(command), T::m_levels(m_level), std::to_string(addr_vec[m_level]))

      if (m_cmd_ready_clk[command] != -1 && clk < m_cmd_ready_clk[command]) {
        // stop recursion: the check failed at this level
        DEBUG_LOG(DNODE, Logging::get("Base"), "\tNot ready! {} < {}", std::to_string(clk), std::to_string(m_cmd_ready_clk[command]))
        return false; 
      }
      ...
      if (child_id < 0 || m_level == m_spec->m_command_scopes[command] || !m_child_nodes.size()) {
        ...
        DEBUG_LOG(DNODE, Logging::get("Base"), "\t Ready! {}", T::m_commands(command))
        return true; 
      }
