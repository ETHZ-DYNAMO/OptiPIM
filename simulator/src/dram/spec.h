...
struct Organization {
  ...
  /// #PEs per bank group: HBM-PIM: each 2 banks share a PE, each bank group has 4 banks
  int m_pe_per_bankgroup = 2;
  /// #Register entries per PE: we assume each PE has #Tensors registers, and each reg.entry is DQ width
  int m_reg_per_pe = 1;
  /// bit precision of PE
  int m_pe_bits = 16;
}; 
