# Ramulator-PIM
Ramulator-PIM extends the original Ramulator by adding PIM commands (`src/base/request.h`) and a frontend to generate PIM commands for machine learning kernel execution (`src/frontend/impl/memory_trace/pim_trace.cpp`).

How to use Ramulator-PIM:
1. Follow the original Ramulator instructions to compile the tool. 
```bash
  $ mkdir build
  $ cd build
  $ cmake ..
  ...
  $ cd ..
```
2. You can run a test by the following commands:

For HBM-PIM:
```bash
  $ ./ramulator2 -f hbmpim_config.yaml
```
For SIMDRAM:
```bash
  $ ./ramulator2 -f simdram_config.yaml
```

3. For now, you can keep using the original `pim_config.yaml` for investigation. Once you feel comfortable with creating new test cases, you can generate a new trace file (follow the format in `pim_inst_new.trace`) and replace `Frontend/path` field in `pim_config.yaml` with your new trace file.
