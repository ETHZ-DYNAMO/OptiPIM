...
from mlir_gen import BertWrapper
...
def main():
    ...
    for layerinfo in model_stats.summary_list:
        if isinstance(layerinfo.module, nn.Conv2d):
            ...
            filename = f"single_layers/alexnet.l{global_idx}.conv{conv_idx}.linalg.mlir"
        ...
        if isinstance(layerinfo.module, nn.Linear):
            ...
            filename = f"single_layers/alexnet.l{global_idx}.fc{conv_idx}.linalg.mlir"
            print(f"Generate {filename}")
            with open(filename, "w") as f:
                f.write(str(layer_mlir))
